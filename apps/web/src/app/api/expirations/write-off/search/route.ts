...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const apiUrl = new URL(`${getApiUrl()}/expirations/write-off/search`);

  for (const parameter of ["query", "storeId", "limit"]) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message = Array.isArray(error.message)
        ...
        : error.message;
      return NextResponse.json(
        { message: message ?? "Não foi possível localizar o produto." },
        { status: response.status },
      );
    }

    return NextResponse.json((await response.json()) as ExpirationRecord[]);
  } catch {
    return NextResponse.json(
